...
@receiver(post_save, sender=CoFounderEarningByProducts)
def after_of_create_co_founder_earning_by_products(sender, instance, created, **kwargs):
    if not created:
        return
    ...
    for user in users:
        object_to_create = DetailCoFounderEarningByProducts()
        ...
        for transaction in transactions_current_month:
            ...
            percentage_to_pay = 1 - (count_days / last_day)
        ...
        for re_investment in re_investments_current_month:
            ...
            earning_to_pay = (
                float(re_investment.amount)
                * float(percentage_to_pay)
                * float(instance.earning_percentage)
            )
            earning += earning_to_pay
        ...
        object_to_create.co_founder_earning_by_products = instance
    ...
    DetailCoFounderEarningByProducts.objects.bulk_create(to_create)
